...
def compute_rollout_attention(all_layer_matrices, start_layer=0):
    ...
    device2 = all_layer_matrices[0].device

    # Créer une matrice identité augmentée pour chaque batch
    eye = torch.eye(num_tokens, device=device2).unsqueeze(0).repeat(1, 1, 1)
    ...
    # Calculer la matrice d'attention globale en partant de la couche spécifiée
...
for i, test_data in enumerate(test_loader):
    ...
    print("Matrice_Rollout : ", Matrice_Rollout.shape)
    ...
    plt.savefig(
        os.path.join("/home/rasri/PycharmProjects/Stage_ASRI_RIDA_INTERPRETABILITY/Heatmap/Attention/Rollout", Name),
        bbox_inches='tight',
        pad_inches=0)
    ...
    colors_df.to_csv(os.path.join(f"/home/rasri/PycharmProjects/Stage_ASRI_RIDA_INTERPRETABILITY/Fichier CSV color "
                                  f"/Attention/Rollout/{Name}"), index=False)
    plt.close()
