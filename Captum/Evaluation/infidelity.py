...
for i, test_data in enumerate(test_loader):
    ...
    noise, perturbed_input = perturb_fn(Volume)
    # Calcul direct des métriques pour l'infidélité
    with torch.no_grad():
        original_output = forward_wrapper(Volume)  # f(x)
        perturbed_output = forward_wrapper(perturbed_input)  # f(x-I)
    # Calculer la différence des sorties perturbées
    perturbed_fwd_diffs = original_output - perturbed_output  # f(x)-f(x-I)
    attr_times_perturb = attributions * noise  # It * fi(f,x)
    attr_times_perturb_sums = attr_times_perturb.view(attr_times_perturb.size(0), -1).sum(dim=-1)
    # Calculer l'infidélité manuellement

    infid = (attr_times_perturb_sums - perturbed_fwd_diffs.view(-1)).pow(2).mean()

    print(Name , " : ", infid.item())
