...
for i, test_data in enumerate(test_loader):
    ...
    baseline = torch.zeros_like(Volume).to(device)

    occ = Occlusion(forward_wrapper)
    attributions = occ.attribute(Volume, target=1, sliding_window_shapes=(1, 32, 32, 19))

    attributions = attributions.squeeze().detach().cpu().numpy()
    attributions = attributions.squeeze().squeeze()
    attributions = attributions.mean(axis=(0, 1))

    y_targets.extend(test_labels_slices)
    y_predictions.extend(attributions)
    ...
    plt.savefig(os.path.join("/home/rasri/PycharmProjects/Stage_ASRI_RIDA_INTERPRETABILITY/Heatmap/Captum/Occlusion", Name),
                bbox_inches='tight',
                pad_inches=0)
    ...
    colors_df.to_csv(os.path.join(f"/home/rasri/PycharmProjects/Stage_ASRI_RIDA_INTERPRETABILITY/Fichier CSV color "
                                  f"/Captum/Occlusion/{Name}"), index=False)
    plt.close()
