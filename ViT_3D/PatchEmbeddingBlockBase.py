...
from typing import Optional, Sequence, Tuple, Union, Type, Any
...
def to_3tuple(x: Any) -> Tuple:
    if isinstance(x, collections.abc.Iterable) and not isinstance(x, str):
        return tuple(x) if len(x) >= 3 else tuple(x) + (x[0],) * (3 - len(x))
    return (x,) * 3
...
class PatchEmbeddingBlock(nn.Module):
    ...
    def __init__(
        ...
        #         print("num patchs", self.num_patches)

        self.pos_embedding_type = pos_embedding_type
        self.pos_embed = look_up_option(pos_embed, SUPPORTED_EMBEDDING_TYPES)

        #         img_size = ensure_tuple_rep(img_size, spatial_dims) 240 -> (240,240,240)
        #         patch_size = ensure_tuple_rep(patch_size, spatial_dims) 10 -> (10,10,10)
        ...
        self.n_patches = np.prod([im_d // p_d for im_d, p_d in zip(self.img_size, self.patch_size)])
        self.patch_dim = int(in_channels * np.prod(self.patch_size))
    ...
    def forward(self, x):
        ...
        # keep/remove position embedding
        if self.pos_embedding_type:
